...
def device(  # noqa C901
    ...
    if dense:
        emb = DenseTableBatchedEmbeddingBagsCodegen(
            [(E, d,) for d in Ds], use_cpu=not torch.cuda.is_available(),
        )
    else:
...
def uvm(
    ...
    emb_uvm = SplitTableBatchedEmbeddingBagsCodegen(
        [(E, d, managed_type, ComputeDevice.CUDA,) for d in Ds[:T_uvm]],
        weights_precision=weights_precision,
        stochastic_rounding=stoc,
        output_dtype=output_dtype,
    ...
    if T_gpu > 0:
        emb_gpu = SplitTableBatchedEmbeddingBagsCodegen(
            [(E, d, EmbeddingLocation.DEVICE, ComputeDevice.CUDA,) for d in Ds[T_uvm:]],
            weights_precision=weights_precision,
            stochastic_rounding=stoc,
        ).cuda()
        ...
        emb_mixed = SplitTableBatchedEmbeddingBagsCodegen(
            [
                (E, d, managed_option, ComputeDevice.CUDA,)
                for (d, managed_option) in zip(
                    Ds, [managed_type] * T_uvm + [EmbeddingLocation.DEVICE] * T_gpu,
                )
            ],
            weights_precision=weights_precision,
            stochastic_rounding=stoc,
...
def cache(  # noqa C901
    ...
    emb_nc = SplitTableBatchedEmbeddingBagsCodegen(
        [(E, d, EmbeddingLocation.MANAGED, ComputeDevice.CUDA,) for d in Ds],
        optimizer=optimizer,
        weights_precision=weights_precision,
        stochastic_rounding=stoc,
    ).cuda()
    ...
    emb = SplitTableBatchedEmbeddingBagsCodegen(
        [(E, d, EmbeddingLocation.MANAGED_CACHING, ComputeDevice.CUDA,) for d in Ds],
        optimizer=optimizer,
        weights_precision=weights_precision,
        stochastic_rounding=stoc,
        cache_load_factor=cache_load_factor,
    ...
    if run_reference:
        ...
        for i in range(runs_of_iters):
            ...
            time_per_iter_refer = benchmark_requests_refer(
                requests, T, B, L, E, D, pooling, weighted, check_median=check_median,
            )
...
def nbit_uvm(
    ...
    emb_uvm = IntNBitTableBatchedEmbeddingBagsCodegen(
        [("", E_uvm, d, weights_precision, managed_type,) for d in Ds[:T_uvm]],
        output_dtype=output_dtype,
        cache_load_factor=cache_load_factor,
        cache_algorithm=cache_alg,
        enforce_hbm=enforce_hbm,
    ...
    if T_gpu > 0:
        emb_gpu = IntNBitTableBatchedEmbeddingBagsCodegen(
            [
                ("", E, d, weights_precision, EmbeddingLocation.DEVICE,)
                for d in Ds[T_uvm:]
            ],
            output_dtype=output_dtype,
            fp8_exponent_bits=fp8_exponent_bits,
        ...
        emb_mixed = IntNBitTableBatchedEmbeddingBagsCodegen(
            [
                ("", e, d, weights_precision, managed_option,)
...
def nbit_cache(  # noqa C901
    ...
    emb_nc = IntNBitTableBatchedEmbeddingBagsCodegen(
        [("", E, d, weights_precision, EmbeddingLocation.MANAGED,) for d in Ds],
        output_dtype=output_dtype,
        enforce_hbm=enforce_hbm,
        fp8_exponent_bits=fp8_exponent_bits,
        fp8_exponent_bias=fp8_exponent_bias,
    ...
    emb = IntNBitTableBatchedEmbeddingBagsCodegen(
        [("", E, d, weights_precision, EmbeddingLocation.MANAGED_CACHING,) for d in Ds],
        cache_load_factor=cache_load_factor,
        cache_algorithm=cache_alg,
        output_dtype=output_dtype,
        enforce_hbm=enforce_hbm,
...
def hashtable(  # noqa C901
    ...
    hash_table = torch.zeros((sum(capacities), 2), dtype=torch.int32,)
    ...
    requests = generate_requests(
        iters, B, T, L, E, requests_data_file=requests_data_file, tables=tables,
    )
    ...
    if use_cpu:
        ...
        time_per_iter = benchmark_requests(
            requests, lambda indices, offsets, _: ht.lookup(indices, offsets),
        )
...
def emb_inplace_update(  # noqa C901
    ...
    update_weight_size = sum(
        [
            rounded_row_size_in_bytes(Ds[t], weights_ty_list[t], row_alignment,)
            for t in update_table_idx
        ]
    )
    ...
    update_table_idx = torch.tensor(
        update_table_idx, device=torch.cuda.current_device(), dtype=torch.int32,
    )
    update_row_idx = torch.tensor(
        update_row_idx, device=torch.cuda.current_device(), dtype=torch.int32,
    )
    update_offsets = torch.tensor(
        update_offsets, device=torch.cuda.current_device(), dtype=torch.int64,
    )
