...
def _get_in_memory_dataloader(args: argparse.Namespace, stage: str,) -> DataLoader:
    ...
    if stage == "train":
        ...
        files = list(filter(is_test_dataset, files))
        rank = dist.get_rank()
        world_size = dist.get_world_size()
        """
        rank = (
    ...
    print(f"[Note]rank:{rank}\t world_size:{world_size}\t stage_files:{stage_files}")
    dataloader = DataLoader(
        InMemoryBinaryCriteoIterDataPipe(
            *stage_files,  # pyre-ignore[6]
            ...
            hot_entries_npy_path=args.hot_entries_path
            if args.use_unified and args.hot_entries_path
            else None,
            n_hot_entries=args.n_hot_entries,
            cold_entries_sharding_type=args.cold_entries_sharding_type,
            use_prefetch=args.use_prefetch if stage == "train" else False,
            stage="test" if stage == "val" else stage,
