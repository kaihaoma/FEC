...
class DeepFM(nn.Module):
    ...
    * On top of the public paper, we allow users to customize the hidden layer
    ...
    def forward(self, deepfm_input: torch.Tensor,) -> torch.Tensor:
        """
        Args:
...
class FactorizationMachine(nn.Module):
    ...
    # def forward(self, embeddings: List[torch.Tensor],) -> torch.Tensor:
    def forward(self, fm_input: torch.Tensor,) -> torch.Tensor:
        """
        Args:
            embeddings: List[torch.Tensor]:
                The list of all embeddings (e.g. dense, common_sparse,
        ...
        cross_term = torch.sum(cross_term, dim=1, keepdim=True) * 1e-4  # [B, 1]
        return cross_term


class CrossNet(nn.Module):
    def __init__(self, in_features, layer_num=2) -> None:
        super().__init__()
        ...
        for i in range(self.kernels.shape[0]):
            nn.init.xavier_normal_(tensor=self.kernels[i])
        for i in range(self.bias.shape[0]):
            nn.init.zeros_(self.bias[i])

    # def forward(self, embeddings: List[torch.Tensor],) -> torch.Tensor:
    def forward(self, cross_input: torch.Tensor,) -> torch.Tensor:

        # cross_input = _get_flatten_input(embeddings)
