...
class Perceptron(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        linear_param = [param.isnan().any() for param in self._linear.parameters()]
        if (
            self._in_size == 512
            and self._out_size == 256
            and (not bool(input.isnan().any()))
            and bool(x.isnan().any())
        ):
            print(
                f"[Note]linear:{self._linear}\t input:{input.isnan().any()}\t param:{linear_param}\t linear:{x.isnan().any()}\t acti:{y.isnan().any()}"
            )
            param_list = [param for param in self._linear.parameters()]
            param_list.extend([input, x, y])
            print(f"[Note]Save to tmp_tensor.pt")
            torch.save(param_list, "tmp_tensor.pt")
        return y
        """
...
class MLP(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        """
        """
        for module in self._mlp:
            input = module(input)
            print(f"[Note]MLP: {module}\t NaN:{input.isnan().any()}")
        return input
        """
