...
class Batch(Pipelineable):
    ...
    def to(self, device: torch.device, non_blocking: bool = False) -> "Batch":
        return Batch(
            ...
            permu=self.permu.to(device, non_blocking=non_blocking),
        )
    ...
    def record_stream(self, stream: torch.cuda.streams.Stream) -> None:
        ...
        # self.permu.record_stream(stream)
...
def train_filter(
    key_fn: Callable[[int], int], train_perc: float, decimal_places: int, idx: int,
) -> bool:
...
def rand_split_train_val(
    datapipe: IterDataPipe, train_perc: float, random_seed: int = 0,
) -> Tuple[IterDataPipe, IterDataPipe]:
    ...
    return (
        _RandFilter(
            datapipe,
            partial(_rand_train_filter_fn, train_perc),
            random.Random(random_seed),
        ),
        _RandFilter(
            datapipe,
            partial(_rand_val_filter_fn, train_perc),
            random.Random(random_seed),
        ),
    )
