...
from torchrec.distributed.batched_embedding_kernel import (
    ...
    BaseUnifiedBatchedEmbeddingBag,
    UnifiedBatchedFusedEmbeddingBag,
    UnifiedBatchedDenseEmbeddingBag,
)
...
def _load_state_dict(
    emb_modules: "nn.ModuleList", state_dict: "OrderedDict[str, torch.Tensor]",
...
class GroupedEmbeddingsLookup(BaseEmbeddingLookup[SparseFeatures, torch.Tensor]):
    def __init__(
        ...
        def _create_lookup(config: GroupedEmbeddingConfig,) -> BaseEmbedding:
            if config.compute_kernel == EmbeddingComputeKernel.BATCHED_DENSE:
                ...
                return BatchedFusedEmbedding(
                    config=config, pg=pg, device=device, fused_params=fused_params,
                )
            elif config.compute_kernel == EmbeddingComputeKernel.DENSE:
                return GroupedEmbedding(
                    config=config, sparse=False, pg=pg, device=device,
                )
            elif config.compute_kernel == EmbeddingComputeKernel.SPARSE:
                return GroupedEmbedding(
                    config=config, sparse=True, pg=pg, device=device,
                )
            else:
                raise ValueError(
        ...
        self.register_buffer(
            "_dummy_embs_tensor",
            torch.empty([0], dtype=torch.float32, device=device, requires_grad=True,),
        )

        self.grouped_configs = grouped_configs

    def forward(self, sparse_features: SparseFeatures,) -> torch.Tensor:
...
class GroupedUnifiedPooledEmbeddingsLookup(
    BaseEmbeddingLookup[SparseFeatures, torch.Tensor]
):
    def __init__(
        self,
        unified_configs: UnifiedEmbeddingConfig,
        device: Optional[torch.device] = None,
        fused_params: Optional[Dict[str, Any]] = None,
        pg: Optional[dist.ProcessGroup] = None,
        feature_processor: Optional[BaseGroupedFeatureProcessor] = None,
    ) -> None:
        def _create_unified_lookup(
            config: UnifiedEmbeddingConfig, device: Optional[torch.device] = None,
        ) -> BaseEmbedding:
            if config.compute_kernel == EmbeddingComputeKernel.BATCHED_DENSE:
                return UnifiedBatchedDenseEmbeddingBag(
                    config=config, pg=pg, device=device,
                )
            elif config.compute_kernel == EmbeddingComputeKernel.BATCHED_FUSED:
                return UnifiedBatchedFusedEmbeddingBag(
                    config=config, pg=pg, device=device, fused_params=fused_params,
                )
            else:
                raise ValueError(
        ...
        super().__init__()
        self._emb_modules: nn.ModuleList = nn.ModuleList()
        self._emb_modules.append(_create_unified_lookup(unified_configs, device))

    def forward(self, sparse_features: SparseFeatures,) -> torch.Tensor:
        assert sparse_features.id_list_features is not None
        assert sparse_features.id_score_list_features is None
        output_embeddings = self._emb_modules[0](sparse_features.id_list_features)
...
class GroupedPooledEmbeddingsLookup(BaseEmbeddingLookup[SparseFeatures, torch.Tensor]):
    def __init__(
        ...
        def _create_lookup(
            config: GroupedEmbeddingConfig, device: Optional[torch.device] = None,
        ) -> BaseEmbedding:
            if config.compute_kernel == EmbeddingComputeKernel.BATCHED_DENSE:
                return BatchedDenseEmbeddingBag(config=config, pg=pg, device=device,)
            elif config.compute_kernel == EmbeddingComputeKernel.BATCHED_FUSED:
                ...
                return GroupedEmbeddingBag(
                    config=config, pg=pg, sparse=False, device=device,
                )
            elif config.compute_kernel == EmbeddingComputeKernel.SPARSE:
                return GroupedEmbeddingBag(
                    config=config, pg=pg, sparse=True, device=device,
                )
            else:
                raise ValueError(
                    f"Compute kernel not supported {config.compute_kernel}"
        ...
        assert len(grouped_score_configs) == 0
    ...
    def forward(self, sparse_features: SparseFeatures,) -> torch.Tensor:
        ...
        if len(self._score_emb_modules) > 0:
            ...
            id_score_list_features_by_group = sparse_features.id_score_list_features.split(
...
class MetaInferGroupedEmbeddingsLookup(
    ...
    def __init__(
        ...
        # return a dummy empty tensor when grouped_configs is empty
        self.register_buffer(
            "_dummy_embs_tensor", torch.empty([0], dtype=torch.float32, device=device,),
        )
...
class InferGroupedLookupMixin(ABC):
    def forward(self, sparse_features: SparseFeaturesList,) -> List[torch.Tensor]:
        embeddings: List[torch.Tensor] = []
