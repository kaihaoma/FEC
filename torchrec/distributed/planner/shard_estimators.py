...
def perf_func_emb_wall_time(
    ...
    """
    # Pooled: True is_weighted: False has_feature_processor False caching_ratio: None,
    shard_perfs = []
    ...
    for hash_size, emb_dim in shard_sizes:
        if (
            ...
            shard_perf = _get_dp_sharding_perf(
                ...
                emb_dim=emb_dim,
                input_data_type_size=input_data_type_size,  # output_data_type_size,
                output_data_type_size=output_data_type_size,
                device_bw=device_bw,
                bw_inter_host=bw_inter_host,
                is_pooled=is_pooled,
...
def calculate_shard_storages(
    ...
    hbm_sizes: List[int] = [
        ...
        for input_size, output_size, hbm_specific_size in zip(
            input_sizes, output_sizes, hbm_specific_sizes,
        )
    ]
    ddr_sizes: List[int] = [
        ...
        for input_size, output_size, ddr_specific_size in zip(
            input_sizes, output_sizes, ddr_specific_sizes,
        )
    ]

    return [
        Storage(hbm=hbm_size, ddr=ddr_size,)
