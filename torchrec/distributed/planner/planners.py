...
class EmbeddingShardingPlanner(ShardingPlanner):
    ...
    def plan(
        ...
        # search_space containing (#emb_tables * #sharding_option * #compute_kernel) sharding options
        ...
        if best_plan:
            for sharding_option in best_plan:
                print(
                    f"[Note] Best plan name:{sharding_option.name}\t sharding_type:{sharding_option.sharding_type}\t KERNEL:{sharding_option.compute_kernel}"
                )
