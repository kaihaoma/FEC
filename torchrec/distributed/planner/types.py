    ...
    def __init__(
        ...
        for rank in range(world_size):
            self._devices.append(
                DeviceHardware(
                    rank=rank, storage=Storage(hbm=hbm_per_device, ddr=ddr_cap),
                )
            )
...
class Shard:
    ...
    def __hash__(self) -> int:
        return hash(
            (tuple(self.size), tuple(self.offset), self.storage, self.perf, self.rank,)
        )
...
class ShardingOption:
    ...
    def __hash__(self) -> int:
        return hash(
            (self.fqn, self.sharding_type, self.compute_kernel, tuple(self.shards),)
        )
...
class Partitioner(abc.ABC):
    ...
    @abc.abstractmethod
    def partition(
        self, proposal: List[ShardingOption], storage_constraint: Topology,
    ) -> List[ShardingOption]:
