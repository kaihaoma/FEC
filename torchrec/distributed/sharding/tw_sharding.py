...
class UnifiedFeaturesDist(BaseSparseFeaturesDist[SparseFeatures]):
    def __init__(
        self, pg: dist.ProcessGroup, device: Optional[torch.device] = None,
    ) -> None:
        super().__init__()
        self._dist = UnifiedFeaturesAllToAll(pg=pg, device=device)
...
class RWPooledEmbeddingDist(BaseEmbeddingDist[torch.Tensor]):
    def __init__(self, pg: dist.ProcessGroup,emb_dim:int) -> None:
        super().__init__()
        self._dist = RWPooledEmbeddingsAllToAll(pg, emb_dim=emb_dim)
    ...
    def forward(
        ...
        return self._dist(
            local_embs, input_batch_size_per_rank, output_batch_size_per_rank
        )
...
class TwPooledEmbeddingDist(BaseEmbeddingDist[torch.Tensor]):
    ...
    def forward(
        ...
        """
        return self._dist(local_embs, batch_size_per_rank)
...
class InferTwSparseFeaturesDist(BaseSparseFeaturesDist[SparseFeaturesList]):
    ...
    def __init__(
        ...
        self._dist: SparseFeaturesOneToAll = SparseFeaturesOneToAll(
            id_list_features_per_rank, id_score_list_features_per_rank, world_size,
        )
