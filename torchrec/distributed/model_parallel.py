...
class DistributedModelParallel(nn.Module, FusedOptimizerModule):
    ...
    def __init__(
        ...
        self.hbm_cap: int = torch.cuda.get_device_properties(self.device).total_memory
        ...
        # [NOTE]self-custom constraint

        if system_name == "FEC":
            custom_constraint = None
        else:
            custom_constraint: Dict[str, ParameterConstraints] = {}
            table_prefix = "t_cat_"
            n_tables = 26 if "criteo" in dataset_name else 21
            tid_list = dataset_to_dptable_id(dataset_name)
            print(
                f"[Note]MP system_name:{system_name}\t n_tables:{n_tables}\t tid_list:{tid_list}"
            )
            for tid in range(n_tables):
                sharding_types = None
                compute_kernels = None
                if system_name == "FLECHE":
                    compute_kernels = [
                ...
                elif system_name == "TorchRec":

                    if tid in tid_list:
                        compute_kernels = [EmbeddingComputeKernel.BATCHED_DENSE.value]
                ...
                if compute_kernels is not None and sharding_types is not None:
                    table_name = table_prefix + str(tid)
                    table_constraint = ParameterConstraints(
                        sharding_types=sharding_types, compute_kernels=compute_kernels,
                    )
                    custom_constraint[table_name] = table_constraint
        ...
        self._unified_cw_config: Optional[UnifiedEmbeddingConfig] = unified_cw_config
        self._unified_dp_config: Optional[UnifiedEmbeddingConfig] = unified_dp_config

        fused_optims = []
        self._init_dmp(fused_optims=fused_optims,)
    ...
    def _init_dmp(self, fused_optims: List[Tuple[str, KeyedOptimizer]],) -> None:
        self._shard_modules_impl(
            self._dmp_wrapped_module, "", fused_optims,
        )
    ...
    def _shard_modules_impl(
        ...
        if sharded_params:
            ...
            sharded_module = self._sharder_map[sharder_key].shard(
                ...
                self._use_unified,
                self._use_prefetch,
                self._unified_cw_config,
                self._unified_dp_config,
            )
            if path:
            ...
            for name, child in module.named_children():
                self._shard_modules_impl(
                    child, path + "." + name if path else name, fused_optims,
                )
    ...
    def _init_parameters(self, module: nn.Module) -> None:
        ...
        def init_parameters(module: nn.Module) -> None:
            ...
            has_reset = hasattr(module, "reset_parameters")
