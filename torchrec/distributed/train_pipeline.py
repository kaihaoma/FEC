...
class UnifiedPipelinedForward(Generic[DistIn, DistOut, Out]):
    def __init__(
        self,
        name: str,
        args: List[ArgInfo],
    ...
    def __call__(self, *input, **kwargs) -> Awaitable[Out]:
        ...
        if self._dist_stream is not None:
            ...
            ctx = self._context.module_contexts[self._name]
            ctx.record_stream(cur_stream)
        # unified_compute_and_output_dist
        return self._module.unified_compute_and_output_dist(
            self._context.module_contexts[self._name], data
        )
...
def _start_data_dist(
    ...
    for module in pipelined_modules:
        ...
        assert isinstance(forward, PipelinedForward) or isinstance(
            forward, UnifiedPipelinedForward
        )
        ...
        # module.input_dist()
        # module.unified_input_dist()
        if use_unified:
            context.input_dist_requests[forward.name] = module.unified_input_dist(
...
def _get_unsharded_module_names_helper(
    model: torch.nn.Module, path: str, unsharded_module_names: Set[str],
) -> bool:
    ...
    for name, child in model.named_children():
        ...
        if isinstance(child, ShardedModule):
            ...
            child_sharded = _get_unsharded_module_names_helper(
                child, curr_path + ".", unsharded_module_names,
            )
            if child_sharded:
                sharded_children.add(name)
    ...
    for node in graph.nodes:
        if node.op == "call_module" and node.target in sharded_modules:
            ...
            if num_found == total_num_args:
                ...
                if use_unified:
                    child.forward = UnifiedPipelinedForward(
    ...
    def _connect(self, dataloader_iter: Iterator[In]) -> None:
        ...
        with torch.cuda.stream(self._memcpy_stream):
            ...
            with torch.no_grad():
                ...
                model = self._model
                # assert self._batch_i.permu is not None
                model(self._batch_i)
                ...
                self._pipelined_modules = _rewrite_model(
                    model, self._context, self._data_dist_stream, self._use_unified,
                )
                print("[Note]Done rewrite model")

        with torch.cuda.stream(self._data_dist_stream):
            _wait_for_batch(batch_i, self._memcpy_stream)
            _start_data_dist(
                self._pipelined_modules, batch_i, self._context, self._use_unified
            )
    ...
    def progress(self, dataloader_iter: Iterator[In]) -> Out:
        ...
        with record_function("## sparse_data_dist ##"):
            with torch.cuda.stream(self._data_dist_stream):
                ...
                _start_data_dist(
                    self._pipelined_modules, batch_ip1, self._context, self._use_unified
                )
