...
# TODO Unified input indices all2all
class UnifiedFeaturesAllToAll(nn.Module):
    def __init__(self, pg: dist.ProcessGroup, device: Optional[torch.device]) -> None:
        super().__init__()

        self._unified_features_all2all: UnifiedAllToAll = UnifiedAllToAll(
            pg=pg, device=device
        )
    ...
    def forward(
        ...
        return SparseFeaturesLengthsAwaitable(
            id_list_features_awaitable=self._unified_features_all2all.forward(
                sparse_features.id_list_features,
            ),
            id_score_list_features_awaitable=None,
        )
...
class SparseFeaturesOneToAll(nn.Module):
    ...
    def __init__(
        ...
        self._id_list_features_one2all: KJTOneToAll = KJTOneToAll(
            id_list_features_per_rank, world_size,
        )
...
class SparseFeaturesListAwaitable(Awaitable[SparseFeaturesList]):
    ...
    def __init__(self, awaitables: List[Awaitable[SparseFeatures]],) -> None:
        super().__init__()
    ...
    def _wait_impl(self) -> SparseFeaturesList:
        ...
        """
        # len(self.awaitables): 2
        # 0: cold entry 1: hot entry
        # ->
        # 0: prefetch cold entry 1: overlap
        sparse_features_list = SparseFeaturesList([w.wait() for w in self.awaitables])
        # separate overlap & toprefetch
        # cold_entries: KeyedJaggedTensor = sparse_features_list[0].id_list_features
...
class SparseFeaturesListIndicesAwaitable(Awaitable[List[Awaitable[SparseFeatures]]]):
    ...
    def __init__(self, awaitables: List[Awaitable[Awaitable[SparseFeatures]]],) -> None:
        super().__init__()
        self.awaitables = awaitables
...
class ListOfSparseFeaturesListAwaitable(Awaitable[ListOfSparseFeaturesList]):
    ...
    def __init__(self, awaitables: List[Awaitable[SparseFeaturesList]],) -> None:
        super().__init__()
        self.awaitables = awaitables
...
class BaseSparseFeaturesDist(abc.ABC, nn.Module, Generic[F]):
    ...
    @abc.abstractmethod
    def forward(self, sparse_features: SparseFeatures,) -> Awaitable[Awaitable[F]]:
        pass
